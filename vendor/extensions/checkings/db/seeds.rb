Refinery::I18n.frontend_locales.each do |lang|
  ...
  if defined?(Refinery::User)
    Refinery::User.all.each do |user|
      if user.plugins.find_by_name('checkings').nil?
        user.plugins.create(:name => "checkings",
...
(Refinery::Checkings::Setting.methods.sort - Refinery::Setting.methods).each do |setting|

    Refinery::Checkings::Setting.send(setting) unless setting.to_s =~ /=$/
